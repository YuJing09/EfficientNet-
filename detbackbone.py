import numpy as np
from modell import EfficientNet,GRU
from bifpn import BiFPN,SeparableConvBlock
...
class EfficientDetBackbone(tf.keras.Model):
  def __init__ (self,compound_coef,num_classes=80,**kwargs):
    ...
    #self.classifier = Classifier(in_channels=self.fpn_num_filters[self.compound_coef], num_anchors=num_anchors,
                                     ...
                                     #pyramid_levels=self.pyramid_levels[self.compound_coef])
    self._blocks=[]
    for i in range(self.pyramid_levels[self.compound_coef]):
      self._blocks.append(GRU())
  ...
  def call(self,input1,input2):
    ...
    return regression
class EBG(tf.keras.Model):
  def __init__ (self,compound_coef,num_classes=80,**kwargs):
    super(EBG, self).__init__()
    cls='efficientnet-b'+str(compound_coef)
    self.compound_coef = compound_coef
    self._block_args,self._global_params=get_model_params(cls,None)
    self.backbone_net=EfficientNet(self._block_args,self._global_params,extract_features=True)
    ...
    #self._gru=[]  
  def call(self,input1,input2):
    features,x=self.backbone_net.call(input1)
    for _ in range(self.fpn_cell_repeats[self.compound_coef]):
      features=self.bifpn[_].call(features)
    #for idx,block in enumerate(self._blocks) :
    out=self._gru(self.flatten(features[-1]),input2)  
      #self._gru.append(block(self.flatten(features[idx]),input2))
...
if __name__ == '__main__':
  ...
  input2=tf.keras.Input(shape=(512))
  output1=model.call(input1,input2)
  
  mm=tf.keras.Model(inputs=[input1,input2],outputs=output1)
  mm.summary()
