...
class FocalLoss():
    def __init__(self):
      self.anchors=Anchors(anchor_scale=4).get_boxes([768,768])
      self.alpha= 0.25
      self.gamma= 2
    def call(self,classifications, regressions,annotations):
      ...
      for j in range(batch_size):
        ...
        bbox_annotation = np.array(annotations[j])
        bbox_annotation = bbox_annotation[bbox_annotation[:, 4] !=-1]
